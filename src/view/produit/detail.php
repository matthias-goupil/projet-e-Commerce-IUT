...
<?php
    ...
    $prix = htmlspecialchars($produit->get("prix"));
    ...
    echo '<p class="scam"> ' . $scam . ' personnes consultent actuellement la page </p>'; // STOCK + 3
        ...
        ?><div><a href="?controller=contenuPanier&action=ajouter&idProduit=<?php echo rawurlencode($id); ?>"><button class="bouton"> Ajouter au panier </button></a></div><?php
    if (Session::userIsCreate() == true) {
        ?>
        <div class = bouton_com ><a href="?controller=avis&action=goToForm&idProduit=<?php echo rawurlencode($id); ?>"><button class="bouton"> Ecrire un commentaire </button></a></div> <?php
    } ?>
    ...
    <?php
     ...
     if($nbAvis > 0) {
        ...
        echo '<p class="note_moy"> Note moyenne : ' . htmlspecialchars($noteGlobale) . '/5</p>';
       }
    ...
    foreach($util as $avis) {
            ...
            echo '<h3 class="nom_prenom">' . htmlspecialchars($nP) .  ' : </h3>';
            echo '<p class="note"> Note : ' . htmlspecialchars($avis["avis"]->get("note")) . ' </p>'; // NOTE AVIS
            if($avis["avis"]->get("commentaire") != null ) {
                echo '<p class="commentaire"> Commentaire : ' . htmlspecialchars($avis["avis"]->get("commentaire")) . '</p>'; // COMMENTAIRE AVIS
            }
            ?>
            </div>
            <?php
